...
def test_encode_default_packet():
    pkt = packet.Packet()
...
def test_decode_default_packet():
    pkt = packet.Packet(encoded_packet=b"6")
    assert pkt.encode(), b"6"


def test_encode_text_packet():
    data = str("text")
    ...
    assert str(pkt) == "Packet(MESSAGE, data=text)"
...
def test_encode_binary_packet():
    ...
    assert str(pkt) == "Packet(MESSAGE, data=<binary>)"


def test_encode_binary_bytearray_packet():
...
def test_encode_binary_b64_packet():
...
def test_encode_binary_packet_py3():
