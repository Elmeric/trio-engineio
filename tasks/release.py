...
def release(c, version):
    ...
    with changelog.open(mode="r+") as fd:
        ...
        compare_ref = re.sub(r"(v[0-9.]+)(...HEAD\n$)", rf"v{version}\g<2>", compare_ref)
    ...
    license_lines[2] = re.sub(r"(^ *Copyright \(c\) *)([0-9-]+)( *.*$)", rf"\g<1>{copyright_year}\g<3>", copyright_line)
    ...
    print()
    print(f"*** Version v{version} has been released and committed: you may now review "
          f"your Changelog ('CHANGELOG.md') and publish with 'invoke publish'.")
    print(f"*** Nothing has been pushed or published yet: you can remove the tag with "
